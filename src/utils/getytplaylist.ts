...
export const getPlayList = (playlistId: string) => new Promise<{
  name?: string;
  list?: videoInfo[];
  err?: string;
}>((res) => {
  ...
  axios.post(`https://music.youtube.com/youtubei/v1/browse?key=${key}&prettyPrint=false`, {
    ...
    try {
      ...
      for (let d of d3) {
        ...
        d4.push({
          title: title,
          duration: (durations[0].length < 2 ? "0" + durations[0] : durations[0]) + ":" + durations[1], // (Number(durations[0]) * 60 + Number(durations[1])).toString(),
          author: author,
          url: url,
